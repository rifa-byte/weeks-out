...
export const PREP_ITEMS: PrepItem[] = [
  ...
  { id: 'cat-cow', name: 'Cat–cow', kind: 'move', dose: '10 slow reps', why: 'Wake the spine up before you brace it.', lifts: 'all' },
  ...
  { id: 'banded-walk', name: 'Banded lateral walk', kind: 'band', dose: '2 × 10 steps each way', why: 'Glute medius on, so the knees track out.', lifts: ['squat', 'deadlift'] },
  { id: 'goblet-hold', name: 'Bottom-position squat hold (goblet or bodyweight)', kind: 'hold', dose: '2 × 20–30 s', why: 'Own the bottom before the bar asks you to.', lifts: ['squat'] },
  ...
  { id: 'empty-bar-squat', name: 'Empty-bar squats, competition tempo', kind: 'move', dose: '2 × 8', why: 'Rehearse the walk-out, the brace and the depth.', lifts: ['squat'], when: 'last' },

  // ----- bench -----
  { id: 'band-pull-apart', name: 'Band pull-aparts', kind: 'band', dose: '2 × 15', why: 'Upper back and rear delts hold the arch.', lifts: ['bench'] },
  ...
  { id: 'bench-arch-hold', name: 'Arch hold on the bench, feet planted', kind: 'hold', dose: '2 × 15 s', why: 'Set-up rehearsal: shoulders pinned, feet driving.', lifts: ['bench'] },
  { id: 'empty-bar-bench', name: 'Empty-bar bench with a pause', kind: 'move', dose: '2 × 10', why: 'Groove the pause and the press command.', lifts: ['bench'], when: 'last' },

  // ----- deadlift -----
  { id: 'hip-hinge-band', name: 'Banded hip hinge (band across the hips)', kind: 'band', dose: '2 × 10', why: 'Teaches the hips to push back, not the knees to bend.', lifts: ['deadlift'] },
  ...
  { id: 'rdl-hold', name: 'Bottom-position RDL hold, empty bar', kind: 'hold', dose: '2 × 15 s', why: 'Find the lat tension and the flat back you want off the floor.', lifts: ['deadlift'] },
  ...
  { id: 'empty-bar-pulls', name: 'Light pulls, perfect set-up', kind: 'move', dose: '2 × 5 at 40–60 kg', why: 'Wedge, tension, then stand. Every warm-up looks like the opener.', lifts: ['deadlift'], when: 'last' },
];

/** Prep list for a session that contains these lifts (accessory-only sessions get the general block). */
export function prepFor(lifts: Lift[]): PrepItem[] {
  const want = new Set<Lift>(lifts.length ? lifts : []);
  const pick = PREP_ITEMS.filter(i => i.lifts === 'all' || i.lifts.some(l => want.has(l)));
